...
void HTMLParser::parseHeadingTag(int level, const QString &attrs, const QString &content, DocumentModel &model) {
    ...
    // 解析富文本内容
    para.runs = parseRichText(content, para.style);
...
QVector<TextRun> HTMLParser::parseRichText(const QString &content, const StyleAttributes &baseStyle) {
    QVector<TextRun> runs;
    
    // 状态机解析：跟踪当前位置和嵌套标签的样式栈
    struct StyleState {
        QString fontFamily;
        int fontSize;
        QString color;
    ...
    currentStyle.fontFamily = baseStyle.fontFamily;
    currentStyle.fontSize = baseStyle.fontSize;
    currentStyle.color = baseStyle.color;
    currentStyle.bold = baseStyle.bold;
    currentStyle.italic = baseStyle.italic;
    currentStyle.underline = baseStyle.underline;
    
    QVector<StyleState> styleStack;
    ...
    // 正则匹配各种行内标签
    QRegularExpression tagRe(R"(<(/?)(\w+)([^>]*)>)", QRegularExpression::CaseInsensitiveOption);
    
    int pos = 0;
    while (pos < remaining.length()) {
        QRegularExpressionMatch match = tagRe.match(remaining, pos);
        
        if (!match.hasMatch() || match.capturedStart() > pos) {
            // 提取标签前的纯文本
            int textEnd = match.hasMatch() ? match.capturedStart() : remaining.length();
            QString plainText = remaining.mid(pos, textEnd - pos);
            ...
            if (!plainText.trimmed().isEmpty() || plainText.contains(' ')) {
                TextRun run;
                run.text = plainText;
                run.fontFamily = styleStack.last().fontFamily;
                run.fontSize = styleStack.last().fontSize;
                run.color = styleStack.last().color;
                run.bold = styleStack.last().bold;
                run.italic = styleStack.last().italic;
                run.underline = styleStack.last().underline;
                runs.append(run);
            }
        }
        ...
        if (isClosing) {
            ...
            // 开始标签：根据标签类型修改样式
            StyleState newStyle = styleStack.last();
            ...
            if (tagName == "b" || tagName == "strong") {
                ...
                if (!styleAttr.isEmpty()) {
                    StyleAttributes inlineStyle = parseInlineStyle(styleAttr);
                    if (inlineStyle.color != "#000000") newStyle.color = inlineStyle.color;
                    if (inlineStyle.fontSize != 12) newStyle.fontSize = inlineStyle.fontSize;
                    if (!inlineStyle.fontFamily.isEmpty() && inlineStyle.fontFamily != "Arial") {
                        newStyle.fontFamily = inlineStyle.fontFamily;
                    }
                    if (inlineStyle.bold) newStyle.bold = true;
                    if (inlineStyle.italic) newStyle.italic = true;
                    if (inlineStyle.underline) newStyle.underline = true;
                }
                // 解析 color 属性 (font 标签)
                QString colorAttr = extractAttribute(attrs, "color");
                if (!colorAttr.isEmpty()) {
                    newStyle.color = colorNameToHex(colorAttr);
                }
                // 解析 face 属性 (font 标签)
                QString faceAttr = extractAttribute(attrs, "face");
                if (!faceAttr.isEmpty()) {
                    newStyle.fontFamily = faceAttr;
                }
                // 解析 size 属性 (font 标签)
                QString sizeAttr = extractAttribute(attrs, "size");
                if (!sizeAttr.isEmpty()) {
                    int size = sizeAttr.toInt();
                    if (size >= 1 && size <= 7) {
                        static const int fontSizes[] = {8, 10, 12, 14, 18, 24, 36};
                        newStyle.fontSize = fontSizes[size - 1];
                    }
                }
            }
    ...
    // 如果没有解析到任何 run，添加一个默认的
    if (runs.isEmpty() && !content.trimmed().isEmpty()) {
        TextRun run;
        run.text = stripHtmlTags(content);
